...
public class PageHandler {
    ...
    public void setPalette(String path) {
        for (Page page : pages.values()) {
            page.getParent().getStylesheets().remove(getClass().getClassLoader().getResource(currentTheme).toExternalForm());
            page.getParent().getStylesheets().add(getClass().getClassLoader().getResource(path).toExternalForm());
            currentTheme = path;
        }
    }
